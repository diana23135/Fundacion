...
const localImagesConnection = require('../services/imagenesLocal');
...
router.delete("/", async (req, res) => {
    ...
    if (taskId && userId) {
        try {
            if (fileName) {
                await localImagesConnection.deleteImage(taskId, userId, fileName);
            }
            const result = await tasks.destroy({ where: { id: taskId } });
            if (result) {
                return res.status(200).json({ message: "Se eliminó con éxito el ID: " + taskId });
            } else {
                return res.status(404).json({ error: "ERROR! No se encontró la tarea" });
            }
        } catch (error) {
            return res.status(500).json({ error: 'Error interno del servidor' });
        }
    } else {
        return res.status(400).json({ error: `ID de la tarea y ID de usuario son requeridos` });
    }
});
...
router.post("/", validateDto(taskDto), async (req, res) => {
    ...
    if (body) {
        try {
            ...
            res.status(500).json({ error: "No se ha podido registrar: " + error });
        }
    } else {
        res.status(400).json({ error: "Datos de la tarea son requeridos" });
    }
});
